	...
	when("#RegisterBuildpack", func() {
		...
		when("registry type is github", func() {
			...
			it("should open a github issue in the browser", func() {
				...
				case "windows":
					execCommand = getFakeExecCommand(t, "url.dll,FileProtocolHandler", expectedURL)
				case "darwin":
...
func getFakeExecCommand(t *testing.T, expectedCmd, expectedURL string) func(command string, args ...string) *exec.Cmd {
	return func(command string, args ...string) *exec.Cmd {
		h.AssertEq(t, command, expectedCmd)
		h.AssertEq(t, args[0], expectedURL)
