...
// 	l.state.Entrypoint("lifecycle/creator").
...
// 		// The [Add] command is reposncible for extracting tar and fetching remote files!
...
// 		// use SecretAsEnv, cause The ENV is exposed in ConfigFile whereas SecretAsEnv not()!
// 		l.state = l.state.Root().SecretAsEnv(fmt.Sprintf("CNB_REGISTRY_AUTH=%s", authConfig))
// 	} else {
...
// 		l.state = l.state.AddVolumes(fmt.Sprintf("%s:%s", launchCache.Name(), mounter.LaunchCacheDir()))
// 	}
// 	l.state = l.state.CMD(additionalFlags...)
