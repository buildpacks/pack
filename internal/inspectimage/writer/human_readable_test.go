	...
	when("Print", func() {
		...
		when("localWithExtension and remoteWithExtension image exits", func() {
			it("prints both localWithExtension and remoteWithExtension image info in a human readable format", func() {
				...
				logger := logging.NewLogWithWriters(&outBuf, &outBuf)
				err := humanReadableWriter.Print(logger, sharedImageInfo, nil, nil, localWithExtensionInfo, remoteWithExtensionInfo, nil, nil)
				assert.Nil(err)
		...
		when("only localWithExtension image exists", func() {
			it("prints localWithExtension image info in a human readable format", func() {
				...
				logger := logging.NewLogWithWriters(&outBuf, &outBuf)
				err := humanReadableWriter.Print(logger, sharedImageInfo, nil, nil, localWithExtensionInfo, nil, nil, nil)
				assert.Nil(err)
		...
		when("only remoteWithExtension image exists", func() {
			it("prints remoteWithExtension image info in a human readable format", func() {
				...
				logger := logging.NewLogWithWriters(&outBuf, &outBuf)
				err := humanReadableWriter.Print(logger, sharedImageInfo, nil, nil, nil, remoteWithExtensionInfo, nil, nil)
				assert.Nil(err)
			...
			when("error handled cases", func() {
				when("there is a remoteErr", func() {
					...
					it("displays the remote error and local info", func() {
						...
						logger := logging.NewLogWithWriters(&outBuf, &outBuf)
						err := humanReadableWriter.Print(logger, sharedImageInfo, nil, nil, localWithExtensionInfo, remoteWithExtensionInfo, nil, remoteErr)
						assert.Nil(err)
				...
				when("there is a localErr", func() {
					...
					it("displays the remote info and local error", func() {
						...
						logger := logging.NewLogWithWriters(&outBuf, &outBuf)
						err := humanReadableWriter.Print(logger, sharedImageInfo, nil, nil, localWithExtensionInfo, remoteWithExtensionInfo, localErr, nil)
						assert.Nil(err)
