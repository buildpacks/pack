...
func testPhases(t *testing.T, when spec.G, it spec.S) {
	...
	when("#Create", func() {
		...
		it("configures the phase with the expected arguments", func() {
			verboseLifecycle := newLifecycle(t, true)
	...
	when("#Analyze", func() {
		...
		when("publish", func() {
			it("runs the phase with the lifecycle image", func() {
				lifecycle := newLifecycle(t, true, func(options *build.LifecycleOptions) {
	...
	when("#Export", func() {
		...
		when("platform api 0.3+", func() {
			...
			it("configures the phase with default arguments", func() {
				lifecycle := newLifecycle(t, true, fakes.WithBuilder(fakeBuilder), func(options *build.LifecycleOptions) {
...
func newLifecycle(t *testing.T, logVerbose bool, ops ...func(*build.LifecycleOptions)) *build.Lifecycle {
