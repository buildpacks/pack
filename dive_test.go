			...
			when(fmt.Sprintf("daemon is %t", useDaemon), func() {
				...
				when("reading image layer data", func() {
					it("has expected layers metadata", func() {
						result, err := subject.Dive("some/image", useDaemon)
						h.AssertNil(t, err)
						h.AssertEq(t, len(result.Layers), 3)
						h.AssertEq(t, result.Layers[0].Digest, "sha256:50644c29ef5a27c9a40c393a73ece2479de78325cae7d762ef3cdc19bf42dd0a")
						h.AssertEq(t, result.Layers[1].Digest, "sha256:8afef23f86669b2edfc71e052427c4e6ef74f9b8b586e8e7732ef2dbb602a401")
						h.AssertEq(t, result.Layers[2].Digest, "sha256:f250386a9c84c89a938f11dac8769830bcbc4f76aa5db6968c945de2de94213c")
					})
				})
			})
		}
