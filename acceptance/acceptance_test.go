			...
			when("build", func() {
				...
				when("default builder is set", func() {
					...
					it("creates a runnable, rebuildable image on daemon from app dir", func() {
						...
						assertImage.HasLabelWithData(repoName, "io.buildpacks.project.metadata", fmt.Printf(`{"project-metadata":{"Source":{"version":["1.0.2"],"metadata":["https://github.com/buildpacks/pack"]}}}`, version, metadata))
