...
func (c *Client) PackageBuildpack(ctx context.Context, opts PackageBuildpackOptions) error {
	...
	packageBuilder := buildmodule.NewBuilder(c.imageFactory)
	...
	bp, err := buildmodule.FromBuildpackRootBlob(mainBlob, writerFactory)
	if err != nil {
	...
	for _, dep := range opts.Config.Dependencies {
		var depBPs []buildmodule.BuildModule
		mainBP, deps, err := c.buildpackDownloader.Download(ctx, dep.URI, buildmodule.DownloadOptions{
		...
		depBPs = append([]buildmodule.BuildModule{mainBP}, deps...)
